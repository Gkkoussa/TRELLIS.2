...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "then generate 6-channel patch features from pure noise on the decoded sparse support."
        )
    )
...
@torch.no_grad()
def generate_sparse_patches_from_noise(
    patch_model,
    patch_config: dict,
    coords: torch.Tensor,
    feats: torch.Tensor,
    ...
    counts = torch.zeros((feats_cpu.shape[0], 1), dtype=torch.float32)

    total_patch_tokens = 0
    used_patches = 0
    skipped_empty = 0

    for batch_start in tqdm(range(0, len(origins), batch_size), desc="Patch generation from full noise"):
        ...
        for origin_tuple in batch_origins:
            ...
            patches.append((local_coords, feats_cpu[selected]))
        ...
        support = make_sparse_batch(patches, device)
        sample = support.replace(torch.randn_like(support.feats))

        if cond_as_token:
            cond = torch.zeros((x.shape[0], 1, 3), dtype=torch.float32, device=device)
        else:
            cond = torch.zeros((x.shape[0], 3), dtype=torch.float32, device=device)
        ...
        for local_idx, selected in enumerate(patch_indices):
            generated_feats = sample_cpu.feats[sample_cpu.layout[local_idx]].float()
            accum[selected] += generated_feats
            counts[selected] += 1.0
            used_patches += 1

    generated = feats_cpu.clone()
    covered = counts.squeeze(1) > 0
    generated[covered] = accum[covered] / counts[covered]
    ...
    stats = {
        ...
        "covered_voxels": int(covered.sum().item()),
        "total_voxels": int(feats_cpu.shape[0]),
        "total_patch_tokens": total_patch_tokens,
    }
    return generated, counts.squeeze(1), stats
...
def save_visualization(
    ...
    generated_coords_gpu = generated_coords.cuda()
    generated_gpu = generated_feats.cuda()
    panels = [
        ...
        ("generated edge", render_views(generated_coords_gpu, generated_gpu, resolution, slice(0, 3), render_resolution, render_ssaa)),
        ...
        ("generated vertex", render_views(generated_coords_gpu, generated_gpu, resolution, slice(3, 6), render_resolution, render_ssaa)),
    ]
...
def main():
    ...
    output_base = Path(args.output_dir).resolve() if args.output_dir else latent_run_dir / "post_decode_patch_generate"
    ...
    for dataset_idx in indices:
        ...
        generated_feats, coverage_counts, patch_stats = generate_sparse_patches_from_noise(
            patch_model,
            patch_config,
            coords,
            feats,
        ...
        save_npz(sample_dir / "patch_generated_512_gaussian_distance.npz", coords, generated_feats)
        np.savez_compressed(
        ...
        if args.save_vxz:
            ...
            save_vxz(sample_dir / "patch_generated_512_gaussian_distance.vxz", coords, generated_feats)
        ...
        save_visualization(
            ...
            coords,
            generated_feats,
            sample_dir / "gt_vs_flow_decoded_vs_patch_generated.png",
            resolution,
        ...
        gt_vs_generated = sparse_l1_metrics(gt_coords, gt_feats, coords, generated_feats)
        sample_metrics = {
            ...
            "patch_generated_num_voxels": int(coords.shape[0]),
            "l1_gt_vs_flow_decoded": gt_vs_flow["l1"],
            "l1_gt_vs_patch_generated": gt_vs_generated["l1"],
            "edge_l1_gt_vs_flow_decoded": gt_vs_flow["edge_l1"],
            "edge_l1_gt_vs_patch_generated": gt_vs_generated["edge_l1"],
            "vertex_l1_gt_vs_flow_decoded": gt_vs_flow["vertex_l1"],
            "vertex_l1_gt_vs_patch_generated": gt_vs_generated["vertex_l1"],
            "union_l1_gt_vs_flow_decoded": gt_vs_flow["union_l1_with_background"],
            "union_l1_gt_vs_patch_generated": gt_vs_generated["union_l1_with_background"],
            "l1_improvement": (
                gt_vs_flow["l1"] - gt_vs_generated["l1"]
                if gt_vs_flow["l1"] is not None and gt_vs_generated["l1"] is not None
                else None
            ),
            "feature_l1_patch_delta": float(torch.mean(torch.abs(generated_feats - feats)).item()) if feats.numel() else None,
            "feature_l2_patch_delta": float(torch.mean((generated_feats - feats).pow(2)).item()) if feats.numel() else None,
            "gt_vs_flow_decoded": gt_vs_flow,
            "gt_vs_patch_generated": gt_vs_generated,

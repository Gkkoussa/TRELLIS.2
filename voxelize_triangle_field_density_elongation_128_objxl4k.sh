#!/bin/bash
#SBATCH --job-name=trifield-density-elong128-objxl4k
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
export FIELD_ROOT="${FIELD_ROOT:-$ROOT/triangle_field_voxels_density_elongation_field}"
...
python data_toolkit/voxelize_triangle_field.py ObjaverseXL \
  --root "$ROOT" \
  --pbr_dump_root "$ROOT" \
  --triangle_field_voxel_root "$FIELD_ROOT" \

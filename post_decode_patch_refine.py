...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "then refine decoded 6-channel voxels with an overlapping 32^3 sparse patch flow."
        )
    )
    ...
    parser.add_argument("--patch_t", type=float, default=0.2)
...
def denoise_sparse_patches(
    ...
    for batch_start in tqdm(range(0, len(origins), batch_size), desc="Patch denoising"):
        ...
        x = make_sparse_batch(patches, device)
        noise = x.replace(torch.randn_like(x.feats))
        t = torch.full((x.shape[0],), patch_t, device=device, dtype=torch.float32)
        t_view = t.view(-1, *[1 for _ in range(len(x.shape) - 1)])
        x_t = (1 - t_view) * x + noise_scale * (sigma_min + (1 - sigma_min) * t_view) * noise
        ...
        sample = x_t
        t_seq = np.linspace(patch_t, 0.0, steps + 1).tolist()
        with amp_context():
        ...
        for local_idx, selected in enumerate(patch_indices):
            refined_feats = sample_cpu.feats[sample_cpu.layout[local_idx]].float()
            accum[selected] += refined_feats
            counts[selected] += 1.0
            used_patches += 1

    refined = feats_cpu.clone()
    covered = counts.squeeze(1) > 0
    refined[covered] = accum[covered] / counts[covered]
    refined = refined.clamp(-1.0, 1.0)
    stats = {
    ...
    return refined, counts.squeeze(1), stats
...
def save_visualization(
    ...
    panels = [
        ...
        ("refined edge", render_views(refined_coords_gpu, refined_gpu, resolution, slice(0, 3), render_resolution, render_ssaa)),
        ...
        ("refined vertex", render_views(refined_coords_gpu, refined_gpu, resolution, slice(3, 6), render_resolution, render_ssaa)),
    ]
...
def main():
    ...
    output_base = Path(args.output_dir).resolve() if args.output_dir else latent_run_dir / "post_decode_patch_refine"
    ...
    for dataset_idx in indices:
        ...
        refined_feats, coverage_counts, patch_stats = denoise_sparse_patches(
            patch_model,
            patch_config,
            coords,
            feats,
        ...
        save_npz(sample_dir / "patch_refined_512_gaussian_distance.npz", coords, refined_feats)
        np.savez_compressed(
        ...
        if args.save_vxz:
            ...
            save_vxz(sample_dir / "patch_refined_512_gaussian_distance.vxz", coords, refined_feats)
        ...
        save_visualization(
            ...
            coords,
            refined_feats,
            sample_dir / "gt_vs_flow_decoded_vs_patch_refined.png",
            resolution,
        ...
        sample_metrics = {
            ...
            "patch_refined_num_voxels": int(coords.shape[0]),
            "l1_gt_vs_flow_decoded": gt_vs_flow["l1"],
            "l1_gt_vs_patch_refined": gt_vs_refined["l1"],
            "edge_l1_gt_vs_flow_decoded": gt_vs_flow["edge_l1"],
            "edge_l1_gt_vs_patch_refined": gt_vs_refined["edge_l1"],
            "vertex_l1_gt_vs_flow_decoded": gt_vs_flow["vertex_l1"],
            "vertex_l1_gt_vs_patch_refined": gt_vs_refined["vertex_l1"],
            "union_l1_gt_vs_flow_decoded": gt_vs_flow["union_l1_with_background"],
            "union_l1_gt_vs_patch_refined": gt_vs_refined["union_l1_with_background"],
            "l1_improvement": (
            ...
            "gt_vs_patch_refined": gt_vs_refined,

#!/bin/bash
#SBATCH --job-name=trellis-postdecode-multidiff
#SBATCH --output=./job_logs/trellis-postdecode-multidiff_%j.log
#SBATCH --nodes=1
...
PATCH_RUN_DIR="${PATCH_RUN_DIR:-/nfs/turbo/coe-jjparkcv-medium/gpranav/objxl_4k/outputs/gaussian_patch_sparse_flow_dit32_noise2.0_51191803}"
...
PATCH_START_T="${PATCH_START_T:-0.8}"
PATCH_STEPS="${PATCH_STEPS:-50}"
PATCH_BATCH_SIZE="${PATCH_BATCH_SIZE:-32}"
PATCH_STRIDE="${PATCH_STRIDE:-16}"
MAX_PATCHES="${MAX_PATCHES:-4096}"
BLEND_MODE="${BLEND_MODE:-triangular}"
...
OUTPUT_DIR="${OUTPUT_DIR:-./post_decode_patch_multidiffusion_outputs_noise2.0}"
RENDER_RESOLUTION="${RENDER_RESOLUTION:-256}"
RENDER_SSAA="${RENDER_SSAA:-2}"
SEED="${SEED:-0}"

CMD=(
  python post_decode_patch_multidiffusion.py

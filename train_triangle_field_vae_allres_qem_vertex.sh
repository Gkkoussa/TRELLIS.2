#!/usr/bin/env bash
#SBATCH --job-name=trifield-vae-qem-vsub-asl
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:2
...
export RUN_NAME_PREFIX="${RUN_NAME_PREFIX:-triangle_field_vae_allres_qem_vertex_subdiv_asl}"
export RUN_NAME="${RUN_NAME:-${RUN_NAME_PREFIX}_${SLURM_JOB_ID}}"
export VERTEX_CONFIG="${VERTEX_CONFIG:-$REPO/configs/scvae/triangle_field_vae_allres_qem_vertex.json}"
export TRAIN_INSTANCES="${TRAIN_INSTANCES:-$ROOT/splits/train_triangle_field_512/instances.txt}"
export NUM_GPUS="${NUM_GPUS:-2}"

if [ ! -f "$VERTEX_CONFIG" ]; then
  echo "Missing vertex-prediction config: $VERTEX_CONFIG" >&2
  exit 1
fi

#!/usr/bin/env bash
#SBATCH --job-name=trifield-vae-qem-vedge
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:2
...
export RUN_NAME_PREFIX="${RUN_NAME_PREFIX:-triangle_field_vae_allres_qem_vertex_edge}"
export RUN_NAME="${RUN_NAME:-${RUN_NAME_PREFIX}_${SLURM_JOB_ID}}"
export EDGE_CONFIG="${EDGE_CONFIG:-$REPO/configs/scvae/triangle_field_vae_allres_qem_vertex_edge.json}"
export TRAIN_INSTANCES="${TRAIN_INSTANCES:-$ROOT/splits/train_triangle_field_512/instances.txt}"
export NUM_GPUS="${NUM_GPUS:-2}"

if [ ! -f "$EDGE_CONFIG" ]; then
  echo "Missing vertex-edge config: $EDGE_CONFIG" >&2
  exit 1
fi
...
python - "$EDGE_CONFIG" "$JOB_CONFIG" "$TRAIN_INSTANCES" \
  "${FINETUNE_ENCODER:-}" "${FINETUNE_DECODER:-}" <<'PY'
import json
import os
import sys

source, destination, train_instances, encoder_ckpt, decoder_ckpt = sys.argv[1:6]
with open(source, "r", encoding="utf-8") as handle:
    config = json.load(handle)
config["dataset"]["args"]["instances_path"] = train_instances
if bool(encoder_ckpt) != bool(decoder_ckpt):
    raise ValueError(
        "Set both FINETUNE_ENCODER and FINETUNE_DECODER, or set neither."
    )
if encoder_ckpt:
    for checkpoint in (encoder_ckpt, decoder_ckpt):

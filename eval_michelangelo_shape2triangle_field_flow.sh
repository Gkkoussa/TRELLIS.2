#!/bin/bash
#SBATCH --job-name=trellis-trifield-flow-shape-eval
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
if [ "$#" -lt 1 ]; then
  echo "Usage: sbatch eval_michelangelo_shape2triangle_field_flow.sh /path/to/flow_run_dir"
  exit 1
fi
...
python /home/koussa/scratch/TRELLIS.2/eval_triangle_field_flow.py \
  --run_dir "$RUN_DIR" \
  --root "$ROOT" \
  --split "$EVAL_SPLIT" \
  --ckpt "$EVAL_CKPT" \

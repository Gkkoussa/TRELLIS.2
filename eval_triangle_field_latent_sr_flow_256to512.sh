#!/bin/bash
#SBATCH --job-name=trellis-trifield-latent-sr-eval256to512
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
...
export EVAL_METRICS_NUM_SAMPLES="${EVAL_METRICS_NUM_SAMPLES:-0}"
...
export EVAL_SAVE_PARTIAL_EVERY="${EVAL_SAVE_PARTIAL_EVERY:-1}"
export EVAL_SEED="${EVAL_SEED:-0}"
export EVAL_RUN_NAME="${EVAL_RUN_NAME:-eval_filtered_${EVAL_SPLIT}_${LOW_RESOLUTION}to${HIGH_RESOLUTION}_latent_flow_step${EVAL_CKPT}_cfg${EVAL_GUIDANCE_STRENGTH}_n${EVAL_NUM_SAMPLES}_${SLURM_JOB_ID:-local}}"
...
echo "Visual samples: $EVAL_NUM_SAMPLES"
echo "Metric samples: $EVAL_METRICS_NUM_SAMPLES (<=0 means all filtered test meshes)"
echo "Batch size: $EVAL_BATCH_SIZE"
echo "Output: ${EVAL_OUTPUT_DIR:-$RUN_DIR/$EVAL_RUN_NAME}"

python -u /home/gpranav/pranav_work/scratch/TRELLIS.2/eval_triangle_field_latent_sr_flow.py \
  --run_dir "$RUN_DIR" \
  --root "$ROOT" \
  --ckpt "$EVAL_CKPT" \
  --split "$EVAL_SPLIT" \
  ...
  --metrics_num_samples "$EVAL_METRICS_NUM_SAMPLES" \
  --batch_size "$EVAL_BATCH_SIZE" \
  --num_workers "$EVAL_NUM_WORKERS" \
  --save_partial_every "$EVAL_SAVE_PARTIAL_EVERY" \

#!/bin/bash
#SBATCH --job-name=trellis-postdecode-generate
#SBATCH --output=./job_logs/trellis-postdecode-generate_%j.log
#SBATCH --nodes=1
...
OUTPUT_DIR="${OUTPUT_DIR:-./post_decode_patch_generate_outputs}"
RENDER_RESOLUTION="${RENDER_RESOLUTION:-256}"
RENDER_SSAA="${RENDER_SSAA:-2}"
SEED="${SEED:-0}"

CMD=(
  python post_decode_patch_generate.py

#!/bin/bash
#SBATCH --job-name=trellis-trifield-vae256-predsub-c64
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:4
...
export RUN_NAME="${RUN_NAME:-triangle_field_vae_256_predsubdiv_c64_${SLURM_JOB_ID}}"
export FLOW_CONFIG="${FLOW_CONFIG:-/home/koussa/scratch/TRELLIS.2/configs/scvae/triangle_field_vae_next_dc_f16c64_fp16_predsubdiv_256_objxl4k.json}"
export BASE_SPLIT_DIR="${BASE_SPLIT_DIR:-$ROOT/splits/train}"
export TRIANGLE_FIELD_VOXEL_DIR="${TRIANGLE_FIELD_VOXEL_DIR:-$ROOT/splits/train/triangle_field_voxels_256}"
...
if [ ! -f "$BASE_SPLIT_DIR/metadata.csv" ]; then
  echo "Missing base metadata: $BASE_SPLIT_DIR/metadata.csv" >&2
  exit 1
fi
